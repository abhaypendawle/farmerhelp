...
exports.generatebill = function (info,customerSSN) {
    ...
    insertInBillPromise.done(function () {
        ...
        getBillIdPromise.done(function (rows) {
            ...
            for (var i=0; i<info.product_details.length; i++) {
                sqlQuery = "INSERT INTO item" +
                    " ( bill_id, product_id, customer_id, quantity, price_per_unit, product_name, product_image_url ) " +
                    "VALUES (" + billId + ", '" + info.product_details[i].product_id + "', '" + customerSSN + "', " + info.product_details[i].quantity + ", " + info.product_details[i].price_per_unit + ",'" + info.product_details[i].product_name + "', '" + info.product_details[i].product_image_url + "');";
                console.log(sqlQuery)
